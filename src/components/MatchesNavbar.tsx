  ...
  return (
    <header className="sticky top-0 z-40 bg-[#1b1843]/80 backdrop-blur-xl border-b border-white/5">
      <div className="mx-auto flex max-w-[1400px] items-center gap-4 px-6 py-4">
        ...
        <nav className="hidden md:flex items-center gap-5 ml-auto">
          {NAV_ITEMS.map(({ label, icon: Icon, to }) => (
            <Link
              key={label}
              to={to}
              className="flex flex-col items-center gap-0.5 text-zinc-400 transition-colors hover:text-white"
            >
              <Icon size={18} />
        ...
        <Link
          to="#"
          className="ml-4 hidden md:flex items-center gap-2 rounded-lg bg-pink-300 px-5 py-2 text-sm font-semibold text-black transition-colors hover:bg-pink-400"
        >
