...
import { ArrowLeft, ArrowFatUp, ArrowFatDown, ChatCircle, Eye, PaperPlaneTilt, } from "@phosphor-icons/react";
...
export default function CommunityPostPage() {
    ...
    if (!post) {
        return (<div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 px-6">
        <h1 className="text-2xl font-bold text-white">Post not found</h1>
        <Link to="/" className="flex items-center gap-2 text-sm text-emerald-400 transition-colors hover:text-emerald-300">
          <ArrowLeft size={16}/> Back to home
        </Link>
      </div>);
    }
    return (<div className="mx-auto max-w-[1400px] px-6 py-8">
      {/* Hero banner */}
      <div className="relative mb-8 overflow-hidden rounded-2xl">
        <img src="/placeholder.webp" alt="Banner" className="h-80 w-full object-cover object-[50%_20%] md:h-96"/>
      ...
      <div className="grid gap-8 lg:grid-cols-[260px_1fr]">
        ...
        <aside className="space-y-1">
          {CATEGORIES.map((cat) => (<div key={cat.name} className="flex items-center justify-between rounded-lg px-4 py-2.5 text-sm text-zinc-400 transition-colors hover:bg-background hover:text-white">
              <span>#{cat.name}</span>
        ...
        <div>
          {/* Back link */}
          <Link to="/" className="mb-6 inline-flex items-center gap-2 text-sm text-zinc-500 transition-colors hover:text-accent">
            <ArrowLeft size={14}/>
            Back to{" "}
            <span className={`rounded-md px-2 py-0.5 text-xs font-medium ${CATEGORY_COLORS[post.categoryColor] ?? CATEGORY_COLORS.emerald}`}>
              #{post.category}
            </span>
          </Link>
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-3 text-xs">
                  <span className={`rounded-md px-2.5 py-1 font-medium ${CATEGORY_COLORS[post.categoryColor] ??
          ...
          <div className="mt-6 rounded-xl border border-white/5 bg-zinc-900/50 p-6">
            ...
            <textarea placeholder="Share your thoughts..." rows={4} className="w-full resize-none rounded-lg border border-zinc-800 bg-background px-4 py-3 text-sm text-white placeholder-zinc-600 outline-none transition-colors focus:border-accent/50"/>
