...
import { ChatCircle, ArrowFatUp, Eye, TrendUp, } from "@phosphor-icons/react";
...
function PostCard({ post }) {
    return (<Link to={`/community/post/${post.id}`} className="group block rounded-xl border border-white/5 bg-[#433f81]/40 p-5 transition-all hover:border-emerald-500/20 hover:bg-[#433f81]">
      {/* Top row: category + meta */}
      <div className="flex items-center gap-3 text-xs">
        <span className={`rounded-md px-2.5 py-1 font-medium ${CATEGORY_COLORS[post.categoryColor] ?? CATEGORY_COLORS.emerald}`}>
          #{post.category}
        </span>
